...
def cov_loss(features):
    ...
    cov = torch.matmul(center_features, center_features.T) / batch_size

    # Compute the squared Frobenius norm of the difference
    decorr_loss = torch.norm(cov, p=2) ** 2 - torch.norm(torch.diag(cov), p=2) ** 2
...
def l2_loss_weights(weights):
    # compute l2 loss per channel
    ...
    for i in range(num_channels):
        ...
        loss += torch.norm(channel_weights, p=2)
...
class XMILoss(nn.Module):
    ...
    def forward(self, output, target, weights, features):
        ...
        loss_l2 = l2_loss_weights(weights) if self.w_l2 != 0 else torch.tensor(0.0, device=output.device)
